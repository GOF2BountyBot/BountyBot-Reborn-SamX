...
Last updated: 2026-07-22 (TRUEUP dispositions: 03 accepted-gap, 04 non-testable, P4 won't-fix; P1 fix in flight). Prior: 2026-06-20 (PvC looting T1–T10 landed on `dev` + documented in T11; added LOOT-deploy follow-up)
...
| TRUEUP-03 | 🔵 | `Ship`/`Module` ARRAY columns block SQLite round-trips (loadout_response fake session, modules builder integration) | **Accepted gap (2026-07-22)**: keep the faithful boundary fakes; rely on the ORM layer for downstream persistence and focus tests on data correctness against the ORM definitions. Revisit only if a src-side ARRAY→JSON variant lands for other reasons |
| TRUEUP-04 | 🔵 | Gateway `_autocomplete_health_probe` is a non-importable nested closure — untestable in place | **Accepted as non-testable (2026-07-22)**: manual-validation steps documented in a comment above the closure in `discord-gateway/src/bot.py` (single-container restart → probe OK; cold start → expected WARNING after 3 attempts) |
...
| #38 | 🔵 | Non-fighter ships (freighters, battlecruisers) render as literal `:colon_code:` instead of an emoji | **Not a code/resolution bug — the assets don't exist.** Verified 2026-07-23 against the prod app (`BOTAPPID` 1379827884851593256, 296 emoji registered): exactly 6 ships have NO emoji in the collection under any name — Midorian/Nivelian/Terran/Vossk Freighter + Terran/Vossk Battlecruiser. Their seed JSONs carry stale CamelCase refs (`<:Midorian_Freighter:769692265694101546>` etc.) whose IDs aren't in the current app, so `data_loader._resolve_emojis` can't rewrite them and the dead ref falls through to a colon-code. Fix = create + upload 6 emoji via `POST /applications/{BOTAPPID}/emojis` with normalizer-friendly names (`midorianfreighter`, `terranbattlecruiser`, …), then re-import ship data. Distinct from the #34 commodity-emoji fix (that was a missing `CommodityRepository` lookup). Check ship `icon`/`assets` seed fields for source art. |
...
| TRUEUP-P5 | 🟠 | `DatabaseManager.table_exists()` always returned `False` — sync `inspect()` from `async def` with no `run_sync` bridge; `MissingGreenlet` swallowed by its own `except SQLAlchemyError` | Fixed 2026-07-22 on `test/suite-mock-true-up`: inspection now runs via `conn.run_sync(...)`; tests assert real True/False both directions + a real (unreachable-path) error branch. Call-site audit: zero production callers today (utility only) — impact was latent, not live. Writeup in `TEST_SUITE_TRUEUP_FOLLOWUPS.md` (R-bc-db-manager) |
...
| TRUEUP-P1 | 🟠 | `LoadoutConsistencyService` evacuate destroyed a legit second copy of a same-name item equipped on two ships | Fixed on branch `fix/loadout-evacuate-item-loss` (PR #64): `equip_one` now enforces invariant I1 at write time (rejects a name already equipped on another ship → ValueError → HTTP 400, same surfacing as B.41); evacuate anti-dup guard retained to repair pre-existing/B.19 phantom dups. Ex-xfail reworked to a conservation test; gate 5562 passed / 0 xfailed. See TRUEUP-P1-sweep for legacy-data audit |
