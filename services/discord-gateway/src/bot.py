...
class GatewayBot(commands.Bot):
    ...
    async def _start_health_server(self, host: str = "0.0.0.0", port: int = 8080):
        ...
        await site.start()
        self.flogger.info(f"Health-check server listening on {host}:{port}")
    ...
    async def _start_fastapi_server(self, host: str = GATEWAY_HOST, port: int = GATEWAY_PORT, access_log: bool = ACCESS_LOG):
        ...
        """
        try:
            self.flogger.info(f"🚀 Starting FastAPI server on {host}:{port}")
            ...
            self.flogger.info(f"✅ FastAPI server task created successfully")
            self.flogger.info(f"📚 API Documentation will be available at: http://{host}:{port}/docs")
            self.flogger.info(f"📖 ReDoc Documentation will be available at: http://{host}:{port}/redoc")
            
        except Exception as e:
            self.flogger.error(f"❌ Failed to start FastAPI server: {e}")
            raise

    async def setup_hook(self):
        self.flogger.info("=== SETUP HOOK STARTED ===")
        self.flogger.debug(f"Python version: {os.sys.version}")
        self.flogger.debug(f"Discord.py version: {discord.__version__}")
        self.flogger.info(f"Bot user: {self.user}")
        self.flogger.debug(f"Bot ID: {self.user.id if self.user else 'Not logged in yet'}")
        ...
        for filename in os.listdir("src/cogs"):
            if filename.endswith(".py") and not any(substring in filename for substring in ("template", "disabled")):
                try:
                    self.flogger.debug(f"Loading cog: {filename}")
                    await self.load_extension(f"cogs.{filename[:-3]}")
                    cog_count += 1
                    self.flogger.info(f"✓ Successfully loaded: {filename}")
                except Exception as e:
                    self.flogger.error(f"✗ Failed to load {filename}: {e}")
                    raise

        self.flogger.info(f"=== SETUP HOOK COMPLETED - Loaded {cog_count} cogs ===")
        # DO NOT call wait_until_ready() here!

    async def on_ready(self):
        self.flogger.info("=== ON_READY EVENT FIRED ===")
        self.flogger.info(f"Bot logged in as: {self.user}")
        self.flogger.debug(f"Bot ID: {self.user.id}")
        self.flogger.debug(f"Bot is in {len(self.guilds)} guilds:")

        for guild in self.guilds:
            self.flogger.trace(f"  - {guild.name} (ID: {guild.id})")
        ...
        self.flogger.info("=== BOT IS FULLY READY ===")

    async def sync_commands(self):
        self.flogger.info("=== STARTING COMMAND SYNC ===")
        try:
            # Show commands before sync
            commands_before = self.tree.get_commands()
            self.flogger.debug(f"Commands before sync: {len(commands_before)}")
            for cmd in commands_before:
                self.flogger.trace(f"  - {cmd.name}: {cmd.description}")
            
            if self.guilds:
                self.flogger.debug("Starting command sync...")
                # Sync to all guilds the bot is in
                self.flogger.debug("=== STARTING GUILD-SPECIFIC SYNC ===")
                for guild in self.guilds:
                    try:
                        self.tree.copy_global_to(guild=guild)
                        synced = await self.tree.sync(guild=discord.Object(id=guild.id))
                        self.flogger.trace(f"✓ Synced {len(synced)} commands to {guild.name}")
                    except Exception as e:
                        self.flogger.error(f"✗ Failed to sync to {guild.name}: {e}")
                        if "403" in str(e):
                            self.flogger.error("Bot missing application.commands scope!")
                        raise
            else:
                # Sync globally (if needed)
                synced = await self.tree.sync()
                self.flogger.trace(f"✓ Synced {len(synced)} commands globally")
            

            self.flogger.info(f"✓ Successfully synced {len(synced)} commands")

            # Log synced commands
            for cmd in synced:
                self.flogger.trace(f"  - Synced: {cmd.name}")

        except Exception as e:
            self.flogger.error(f"✗ Failed to sync commands: {e}")
            self.logger.exception("Command sync error details:")
            raise

    async def on_error(self, event, *args, **kwargs):
        self.flogger.error(f"Error in event {event}")
        self.logger.exception("Event error details:")

    async def on_command_error(self, ctx, error):
        self.flogger.error(f"Command error in {ctx.command}: {error}")
        self.logger.exception("Command error details:")

    async def close(self):
        """Enhanced close method to properly shutdown FastAPI server."""
        self.flogger.info("🛑 Shutting down bot and FastAPI server...")
        
        # Cancel FastAPI task if it exists
        if self.fastapi_task and not self.fastapi_task.done():
            self.flogger.info("🔄 Cancelling FastAPI server task...")
            self.fastapi_task.cancel()
            try:
                await self.fastapi_task
            except asyncio.CancelledError:
                self.flogger.info("✅ FastAPI server task cancelled")
        
        # Call parent close method
        await super().close()
        self.flogger.info("👋 Bot shutdown complete!")
