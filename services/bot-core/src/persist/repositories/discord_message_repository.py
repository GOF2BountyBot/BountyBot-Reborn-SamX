...
class DiscordMessageRepository(GenericRepository[DiscordMessage]):
    ...
    async def create_or_update(
        ...
        """
        flogger.trace(f"Creating or updating Discord message: guild_id={raw.get('guild_id')}, "
        ...
        try:
            ...
            if existing:
                ...
                flogger.debug(f"Updated Discord message {existing.id}: type='{existing.message_type}', "
            ...
            flogger.debug(f"Created Discord message {message.id}: type='{message.message_type}', "
                          f"guild_id={guild_id}, channel_id={channel_id}, message_id={message_id}")
            return message
    ...
    async def get_by_composite_key(
        ...
        """
        flogger.trace(f"Querying message with guild_id={guild_id}, channel_id={channel_id}, "
    ...
    async def get_by_type(
        ...
        """
        flogger.trace(f"Querying messages by type='{message_type}', guild_id={guild_id}, "
    ...
    async def list_by_guild_and_type(
        ...
        messages = await self.get_by_type(
            db,
            message_type,
            guild_id=guild_id
        )
    ...
    async def delete_by_composite_key(
        ...
        """
        flogger.trace(f"Attempting to delete message with guild_id={guild_id}, "
        ...
        if message:
            try:
                ...
                await db.commit()
                flogger.debug(f"Deleted Discord message: {message.id} (guild_id={guild_id}, "
                              f"channel_id={channel_id}, message_id={message_id})")
                return True
            except Exception as e:
                ...
                await db.rollback()
                raise
        flogger.trace(f"No message found to delete for guild_id={guild_id}, "
                      f"channel_id={channel_id}, message_id={message_id}")
        return False
