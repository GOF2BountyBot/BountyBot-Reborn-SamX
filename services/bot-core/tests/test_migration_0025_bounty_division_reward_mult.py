"""Migration tests for 0025_bounty_division_reward_mult.

Verifies that:
- upgrade() adds the nullable bounty_division_reward_mult JSONB column to guild_configs
...
SQLite approach (matching test_migration_0023 pattern):
...
Postgres path (matching test_migration_0023 pattern):
...
_NEW_COLS = ("bounty_division_reward_mult",)
...
_MIGRATION_PATH = os.path.abspath(
    os.path.join(
        ...
        "revisions",
        "versions",
        "0025_bounty_division_reward_mult.py",
    )
)


def _load_migration_module():
    """Load the 0025 migration module via importlib (avoids Alembic env)."""
    spec = importlib.util.spec_from_file_location("migration_0025_real", _MIGRATION_PATH)
...
class TestMigration0025Structure:
    """0025 structural tests using in-memory SQLite.
    ...
    def test_upgrade_adds_column(self):
        """upgrade() adds the bounty_division_reward_mult column."""
    ...
    def test_downgrade_drops_column(self):
        """downgrade() removes the bounty_division_reward_mult column."""
        engine = create_engine("sqlite:///:memory:")
        _create_guild_configs_table(engine)
...
@_PG_MARK
class TestMigration0025Postgres:
    """0025 Postgres tests — run against the live Postgres, always rolled back.
    ...
    def test_upgrade_column_is_nullable(self, pg_engine):
        """After upgrade(), bounty_division_reward_mult exists and is nullable."""
        mod = _load_migration_module()
    ...
    def test_downgrade_drops_column(self, pg_engine):
        """After upgrade() + downgrade(), bounty_division_reward_mult is absent."""
        mod = _load_migration_module()
