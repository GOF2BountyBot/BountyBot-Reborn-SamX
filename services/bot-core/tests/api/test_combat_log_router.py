...
class TestListCombatLog:
    ...
    def test_duel_type_forwarded(self, client, mock_service):
        ...
        for dt in ("pvp", "bounty"):
            resp = client.get("/api/v1/combat-log", params={"user_id": 100, "guild_id": 999, "duel_type": dt})
            assert resp.status_code == 200
    ...
    def test_invalid_duel_type_returns_422(self, client):
        ...
        resp = client.get("/api/v1/combat-log", params={"user_id": 100, "guild_id": 999, "duel_type": "nonsense"})
        assert resp.status_code == 422
