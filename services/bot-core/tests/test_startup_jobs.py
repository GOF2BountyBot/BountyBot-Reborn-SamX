...
class TestDefaultSchedulerJobsConstant:
    ...
    def test_three_jobs_defined(self):
        """DEFAULT_SCHEDULER_JOBS contains exactly five job definitions (pg_backup_default added)."""
...
class TestRegisterDefaultJobs:
    ...
    def test_three_jobs_added_on_clean_scheduler(self):
        """All five default jobs are added when the scheduler has no prior jobs."""
    ...
    def test_no_jobs_added_when_all_already_exist(self):
        """add_job is never called when all five default jobs already exist."""
        scheduler = _make_mock_scheduler(
            existing_job_ids=[
                "bounty_spawn_default",
                "shop_refresh_default",
    ...
    def test_only_missing_jobs_are_added_partial_existing(self):
        """Only the four missing jobs are added when one already exists."""
    ...
    def test_single_missing_job_added(self):
        """Only the one missing job is added when four already exist."""
        scheduler = _make_mock_scheduler(
            existing_job_ids=[
                "bounty_spawn_default",
                "shop_refresh_default",
