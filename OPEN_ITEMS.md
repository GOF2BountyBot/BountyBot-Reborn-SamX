...
| B.83 | RESOLVED | 🟢 | Intermittent: expired bounties occasionally not auto-cleaned (Discord post not deleted) | **Fixed**: Added `bounty_failsafe_cleanup_executor` — hourly Discord-first sweep that fetches messages from bounty channels, classifies each post against the DB (live/expired/captured/orphan), and deletes stale posts + DB records. Registered as `bounty_failsafe_cleanup_default` cron job at :30 past every hour. 12 tests in `tests/test_bounty_failsafe_cleanup_executor.py`. |
