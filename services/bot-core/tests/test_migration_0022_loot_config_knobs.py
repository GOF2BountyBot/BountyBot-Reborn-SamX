"""Migration tests for 0022_loot_config_knobs (LOOT_JOURNAL §8 / T2).

Verifies that:
- upgrade() adds all 19 nullable per-guild loot-override columns to guild_configs
  (idempotent via inspector guard)
- downgrade() drops all 19 columns (idempotent via inspector guard)
- The REAL upgrade()/downgrade() functions run without error (smoke test)
- Re-running upgrade()/downgrade() is a safe no-op

SQLite approach (matching test_migration_0020 pattern):
...
Postgres path (matching test_migration_0020 pattern):
...
_MIGRATION_PATH = os.path.abspath(
    os.path.join(
        ...
        "revisions",
        "versions",
        "0022_loot_config_knobs.py",
    )
)


def _load_migration_module():
    """Load the 0022 migration module via importlib (avoids Alembic env)."""
    spec = importlib.util.spec_from_file_location("migration_0022_real", _MIGRATION_PATH)
...
class TestMigration0022Structure:
    """0022 structural tests using in-memory SQLite.

    These verify add/drop idempotency and column existence without requiring
    a live Postgres instance.
    """

    def test_upgrade_adds_all_nineteen_columns(self):
        """upgrade() adds all 19 loot-knob override columns."""
    ...
    def test_downgrade_drops_all_nineteen_columns(self):
        """downgrade() removes all 19 override columns."""
        engine = create_engine("sqlite:///:memory:")
        _create_guild_configs_table(engine)
...
def _build_mock_op_pg(conn: sa.engine.Connection) -> MagicMock:
    ...
    Renders the real column type so Integer/Float columns add correctly. All
    columns are nullable in 0022, so no NOT NULL/DEFAULT handling is needed.
    """
    mock_op = MagicMock()
    mock_op.get_bind.return_value = conn
...
@_PG_MARK
class TestMigration0022Postgres:
    """0022 Postgres tests — run against the live Postgres, always rolled back.
    ...
    def test_upgrade_columns_are_nullable(self, pg_engine):
        """After upgrade(), all 19 columns exist and are nullable in information_schema."""
        mod = _load_migration_module()
    ...
    def test_downgrade_drops_all_columns(self, pg_engine):
        """After upgrade() + downgrade(), all 19 columns are absent."""
        mod = _load_migration_module()
