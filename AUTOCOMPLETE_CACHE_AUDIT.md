...
| 3 | shopCog | `tier_autocomplete` | `/buy`(tier?) | static `_valid_tiers` (list) | none (enum) | never | static-list | **D (no refresh_fn needed — frozen enum)** | 3 ✅ N/A |
| 4 | shopCog | `item_type_autocomplete` | `/shop` | static `_valid_item_types` (list) | none (enum) | never | static-list | **D (no refresh_fn needed — frozen enum)** | 3 ✅ N/A |
...
| 14 | bountyCog | `system_autocomplete` | `/check` | static `_systems_cache "all"` (TTL=None, **no refresh_fn**) | preload `_preload_data` | never (catalog) | static-peek | **D + self-heal refresh_fn** | 3 ✅ DONE |
...
| 17 | adminCog | `render_setting_autocomplete` | `/admin_render_config` | static `_render_settings` (list) | preload `_preload_render_settings` | never (catalog) | static-list | **D (no refresh_fn needed — frozen enum)** | 3 ✅ N/A |
| 18 | adminCog | `tier_autocomplete` | admin tier cmds | static `_valid_tiers` (list) | none (enum) | never | static-list | **D (no refresh_fn needed — frozen enum)** | 3 ✅ N/A |
| 19 | adminCog | `item_name_autocomplete` | `/admin_give_item` | `_item_catalog` per-category (TTL=None, **no refresh_fn**) | `_preload_static_catalogs` | never (catalog) | static-peek (via `.get`) | **D + self-heal refresh_fn** | 3 ✅ DONE |
...
| 21 | adminCog | `game_ship_autocomplete` | `/admin_*ship*` | `_ship_catalog "all"` (TTL=None, **no refresh_fn**) | `_preload_static_catalogs` | never (catalog) | static-peek (via `.get`) | **D + self-heal refresh_fn** | 3 ✅ DONE |
...
| 23 | adminCog | `constants_autocomplete` | `/admin_config_constants` | static `_GAME_CONSTANT_FIELDS` (tuple) | none (in-code) | never | static-list | **D (no refresh_fn needed — frozen enum)** | 3 ✅ N/A |
...
| 26 | playerCog | `tier_autocomplete` | `/leaderboard` | static `_valid_tiers` (list) | none (enum) | never | static-list | **D (no refresh_fn needed — frozen enum)** | 3 ✅ N/A |
| 27 | skinsCog | `ship_autocomplete` | `/ship_skin` | `_ship_skins.keys()` (TTL=None, **no refresh_fn**) | `_preload_ship_skins` | never (catalog) | static-peek | **D + self-heal (size-guard, see §4)** | 3 ✅ DONE |
| 28 | skinsCog | `skin_autocomplete` | `/ship_skin` | `_ship_skins[ship]` keyed by ship name | `_preload_ship_skins` | never (catalog) | static-peek | **D + self-heal refresh_fn (per-ship)** | 3 ✅ DONE |
| 29 | skinsCog | `skinnable_ship_autocomplete` | `/render_ship` etc. | `_ship_skins.keys()` + `_ship_render_info` dict | `_preload_ship_skins` | never (catalog) | static-peek | **D + self-heal (size-guard, see §4)** | 3 ✅ DONE |
| 30 | aboutCog | `category_autocomplete` | `/about` | `_categories_cache "all"` (TTL=None, **no refresh_fn**) | `_preload_data` | never (catalog) | static-peek | **D + self-heal refresh_fn** | 3 ✅ DONE |
| 31 | aboutCog | `system_autocomplete` | `/about`(legacy) | `_objects_cache "system"` | `_preload_data` | never (catalog) | static-peek | **D + self-heal refresh_fn** | 3 ✅ DONE |
| 32 | aboutCog | `object_autocomplete` | `/about` | `_objects_cache[category]` keyed by category | `_preload_data` | never (catalog) | static-peek | **D + self-heal refresh_fn (per-category)** | 3 ✅ DONE |
| 33 | devCog | `category_autocomplete` | `/load_data` | static `_categories` (list, preloaded) | `_preload_categories` | on `/load_data` schema change (rare) | static-list | **D + self-heal (size-guard reload)** | 3 ✅ DONE |
| 34 | helpCog | `_user_category_autocomplete` | `/help` | in-code tuple `_USER_CATEGORY_ORDER` | none (in-code) | never | static-list | **D (no refresh_fn — frozen in-code)** | 3 ✅ N/A |
| 35 | helpCog | `_admin_category_autocomplete` | `/admin_help` | in-code tuple `_ADMIN_CATEGORY_ORDER` | none (in-code) | never | static-list | **D (no refresh_fn — frozen in-code)** | 3 ✅ N/A |
