...
| 22 | adminCog | `player_ship_autocomplete` | `/admin_remove_ship` | **target** player `(g,target_u)` + **live GET /ships/player** | resolve helper (cold-fill) + **HTTP per keystroke** | target's ships: setactive/sell-ship/give-ship/admin-remove-ship | resolve cold-fill **+ live-HTTP** | **C → reuse `ships_cache (g,target_pid)`** | 2 |
...
| 24 | adminCog | `admin_duel_autocomplete` | `/admin_duel` | guild-scoped pending duels — **live GET /duels/pending-all** | **HTTP per keystroke** | challenge/accept/reject/cancel/admin-cancel/expire (guild) | **live-HTTP** | **C → NEW `_admin_pending_duel_cache (g)`** | 2 |
| 25 | combatLogCog | `battle_autocomplete` | `/combat-log` | per-user — **live GET /combat-log** | **HTTP per keystroke** | fight finished for that user: PvC `/check`, PvP `/duel-accept` | **live-HTTP** | **C → NEW per-user `combatlog_cache (g,u)`** | 2 |
