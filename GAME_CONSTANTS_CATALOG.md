...
| `DIVISION_TL_CENTERS` | dict[str, int] | `{bronze:1, silver:3, gold:6, platinum:8}` | global (flatten candidate) | — | Per-division centre TL passed to pick_random_item_tl() when spawning criminal enemies; consumed in bounty_service alongside DIVISION_MAX_TL. A flatten refactor (issue #70) would split this into four scalar per-guild columns. | (Global — sets the target equipment tech level for criminals in each division; not yet per-guild.) |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `BOUNTY_REWARD_TO_XP_GAIN_MULT` | float | 0.1 | per-guild (wired) | 0.0–100.0 | Multiplies the winner's total credit payout to yield XP; applied in bounty_service as `xp = int(total_winner_credits × mult)` after the winner-reserve split. | How much XP players earn per credit gained from capturing a bounty (0.1 = 1 XP per 10 credits). |
| `BOUNTY_WINNER_RESERVE_FACTOR` | float | 0.25 | per-guild (wired) | 0.0–1.0 | Fraction of the total prize pool set aside as the winner's guaranteed payout; the remainder is the consolation pool split across route systems in bounty_service.resolve_bounty. | The share of a bounty's prize pool guaranteed to the player who caught the criminal, with the rest split among other checkers. |
| `BOUNTY_DIVISION_REWARD_MULT` | dict[str, float] | `{bronze:1.0, silver:2.0, gold:1.0, platinum:1.0}` | per-guild (JSONB, wired) | ≥0.0 per div (proposed ≤10.0) | Scales the full prize pool before the winner-reserve split in bounty_service._spawn_bounty; silver defaults to 2.0 to remove the dead-rung payout issue where silver rewards were near-equal to bronze despite a harder fight. | A per-division multiplier on bounty prize pools — silver defaults to 2× so rewards match difficulty. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `MAX_BOUNTIES_PER_DIVISION` | int | 5 | convert planned | 1–20 (proposed) | Upper cap in temperature_service.get_target_bounty_count: `min(MAX_BOUNTIES_PER_DIVISION, max(1, int(temperature)))`; limits concurrent live bounties per division guild-wide. | The maximum number of active bounties allowed in any single division at the same time. |
| `CLOSE_BOUNTY_THRESHOLD` | int | 4 | per-guild (wired) | 1–50 | Number of route stops ahead of the current system at which bounty_service._format_check_result displays a "close" proximity hint; resolved via resolve_constant. | How many systems away a criminal must be before players see a "close" hint in their /check result. |
| `SHIP_VALUE_REWARD_PERCENTAGE` | float | 0.01 | retiring (dead) | — | Originally 1% of the criminal's ship credit value added to the bounty reward; no live code path calls it — predates the current loadout-value reward formula and has no callers. | (Retiring — not currently applied to any bounty rewards.) |
| `CRIMINAL_EQUIP_DAMAGELESS_WEAPON_CHANCE` | int (%) | 20 | retiring (dead) | — | Intended chance that a criminal receives a damageless weapon; superseded by the CRIMINAL_EXCLUDE_EMP_WEAPONS toggle and no longer called from any loadout assembly path. | (Retiring — has no effect on current criminal loadouts.) |
| `CRIMINAL_MAX_GEAR_UPGRADE` | int | 1 | per-guild (wired) | 0–10 | Maximum TL steps above the criminal's base TL that spawned weapons and modules may be; resolved via resolve_constant in bounty_service._assemble_criminal_loadout. | How many tech levels above a criminal's base level its weapons and modules can reach during spawn. |
...
| `BOUNTY_WAYPOINT_MIN_DEGREE` | int | 2 | per-guild (column live; API exposure planned) | 1–10 (proposed) | Minimum gate-connected neighbours a candidate waypoint system must retain after earlier-leg systems are removed; validated in bounty_service._validate_waypoint to prevent isolated waypoints. | The minimum number of connections a waypoint system must have to be used — prevents criminals from being routed through dead-end systems. |

### Criminal Loadout — Primary Weapons & Range

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `LONG_RANGE_THRESHOLD_M` | int (metres) | 2600 | per-guild (wired) | 0–50000 | Cutoff in metres; a primary weapon whose range_m exceeds this value is classified as LONG-range in bounty_service._select_primary_weapons, feeding the CRIMINAL_LONG_RANGE_PCT floor calculation. | The weapon range (in metres) above which a primary weapon counts as "long-range" when arming a criminal. |
| `CRIMINAL_LONG_RANGE_PCT` | float | 0.50 | per-guild (wired) | 0.0–1.0 | Floor share of long-range primaries per criminal ship computed as `ceil(pct × max_primaries)`; remaining slots use a per-slot roll in bounty_service._select_primary_weapons. | The minimum fraction of a criminal's primary weapons that must be long-range (50% by default). |
| `PRIMARY_TL_BAND_WEIGHTS` | dict[str, int] | `{center:70, minus1:20, plus1:10}` | per-guild (JSONB, wired) | ≥0 per key (proposed ≤1000) | Relative pick weights for the ±1 TL band around the target TL when selecting criminal primaries in bounty_service._select_primary_weapons; keys must be exactly {center, minus1, plus1}. | How likely criminals are to carry weapons exactly at their tech level versus one level below or above (defaults favour exact-TL weapons 70% of the time). |

### Criminal Loadout — Module Gate Chances

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `CRIMINAL_CLOAK_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:0, silver:25, gold:66, platinum:100}` | per-guild (JSONB, wired) | 0–100 per div | Chance (%) that a criminal receives a CloakModule during Gate-1 equip in bounty_service._assemble_criminal_loadout; Bronze criminals never cloak, Platinum always do. | The percentage chance a criminal has a cloaking device per division — 0 means never, 100 means always. |
| `CRIMINAL_BOOSTER_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:50, silver:100, gold:100, platinum:100}` | per-guild (JSONB, wired) | 0–100 per div | Chance (%) that a criminal receives a BoosterModule in Gate-1 equip; Bronze at 50% gives new players a manageable fight while Silver+ always field one. | The percentage chance a criminal has a weapon booster per division. |
| `CRIMINAL_EMERGENCY_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:0, silver:25, gold:50, platinum:100}` | per-guild (JSONB, wired) | 0–100 per div | Chance (%) that a criminal receives an EmergencySystemModule in Gate-1 equip in bounty_service._assemble_criminal_loadout; scales with division difficulty. | The percentage chance a criminal has an emergency survival system per division — higher values make criminals harder to destroy. |
| `CRIMINAL_WEAPONMOD_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:0, silver:25, gold:50, platinum:100}` | per-guild (JSONB, wired) | 0–100 per div | Chance (%) that a criminal receives a PrimaryWeaponModModule in Gate-1 equip; scales weapon lethality per tier in bounty_service._assemble_criminal_loadout. | The percentage chance a criminal has a weapon damage modifier per division. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `GUILD_ACTIVITY_DECAY_RATE` | float | ~0.667 (2/3) | per-guild (wired) | 0.0–1.0 | Geometric decay multiplier applied once per GUILD_ACTIVITY_DECAY_INTERVAL in temperature_service.decay: `temperature ← max(min_activity, temperature × rate)`; resolved via resolve_constant. | How fast guild activity cools down when no /check uses happen — lower values mean bounty spawning slows more aggressively during quiet periods. |
| `MIN_GUILD_ACTIVITY` | float | 1.0 | per-guild (wired) | 0.0–100.0 | Floor clamp applied after each decay step and in temperature_service.get_target_bounty_count; ensures activity never reaches zero, preventing bounty spawning from stopping entirely. | The minimum activity level your guild maintains even during long quiet periods — prevents bounty spawning from stopping entirely. |
| `ACTIVITY_TEMP_PER_PLAYER` | int | 1 | per-guild (override currently broken; wiring fix planned) | 0–100 | Activity increment per /check use; temperature_service.heat reads `GameConstants.ACTIVITY_TEMP_PER_PLAYER` directly rather than through resolve_constant, so the GuildConfig column has no effect until the wiring fix lands. | How much each /check use raises the guild's activity level — currently fixed at 1 regardless of your guild setting until a fix is deployed. |

### Bounty Spawn Timing

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `BOUNTY_DELAY_RANDOM_MIN` | int (minutes) | 5 | per-guild (wired) | 0–1440 | Lower bound of the random spawn-delay window sampled by temperature_service.sample_spawn_delay after a bounty fires; cross-validated against BOUNTY_DELAY_RANDOM_MAX in config_schema. | The minimum number of minutes between bounty spawns. |
| `BOUNTY_DELAY_RANDOM_MAX` | int (minutes) | 7 | per-guild (wired) | 0–1440 | Upper bound of the spawn-delay window in temperature_service.sample_spawn_delay; must be ≥ BOUNTY_DELAY_RANDOM_MIN (cross-field validator in config_schema). | The maximum number of minutes between bounty spawns. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `GUILD_ACTIVITY_DECAY_INTERVAL` | int (seconds) | 3600 | global-infra | — | APScheduler trigger interval for the activity-decay job in main.py; not per-guild, set once at startup via the `BOUNTYBOT_GUILD_ACTIVITY_DECAY_INTERVAL` env var. | (Global infrastructure — controls how often the activity decay runs; not a per-guild setting.) |
...
| `AUDIT_RETENTION_DAYS` | int (days) | 30 | global-infra (retention) | — | Age threshold for pruning AdminAuditLog rows in db_retention_executor and admin_audit_log_repository; full audit history is preserved separately via scheduled pg_backup. | (Global — controls how long admin action logs are retained in the live database before being purged; backups are kept separately.) |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `DUEL_LOG_MAX_LENGTH` | int | 10 | retiring (dead) | — | Originally capped the number of entries in a duel combat log; no production code reads it since TickResolver replaced SimpleTTKResolver in T10. Safe to remove after config_schema field is dropped. | (Retiring — has no effect on current duel logs.) |
| `DUEL_CLOAK_CHANCE` | int (%) | 20 | retiring (dead) | — | Formerly the chance (%) of a cloak trigger in a duel; superseded by the Phase-1 accuracy-based cloak model in T10's TickResolver. The config_schema field still exists but no resolver reads it. | (Retiring — has no effect on current duel outcomes.) |
...
| `SHOP_REFRESH_INTERVAL` | `int` | `21600` | retiring | — | Was the refresh cadence in seconds (6 hours); now dead code — shop refresh is driven entirely by the scheduled-jobs system and this value is never read by any active path. | No longer has any effect; shop refresh timing is controlled by the scheduled-jobs configuration. |
| `SHOP_DEFAULT_SHIPS_NUM` | `int` | `5` | retiring | — | Former default count of ships stocked per refresh; superseded by the per-guild `item_count_ranges` JSON in `UpdateShopConfigRequest` and is no longer read by `shop_service`. | No longer has any effect; ship count per refresh is set in the shop stock configuration. |
| `SHOP_DEFAULT_WEAPONS_NUM` | `int` | `5` | retiring | — | Former default weapon count per refresh; same retirement path as `SHOP_DEFAULT_SHIPS_NUM`; no active consumer in `shop_service`. | No longer has any effect; weapon count per refresh is set in the shop stock configuration. |
| `SHOP_DEFAULT_MODULES_NUM` | `int` | `5` | retiring | — | Former default module count per refresh; same retirement path as `SHOP_DEFAULT_SHIPS_NUM`. | No longer has any effect; module count per refresh is set in the shop stock configuration. |
| `SHOP_DEFAULT_TURRETS_NUM` | `int` | `2` | retiring | — | Former default turret count per refresh; same retirement path as `SHOP_DEFAULT_SHIPS_NUM`. | No longer has any effect; turret count per refresh is set in the shop stock configuration. |
| `SHOP_DEFAULT_TOOLS_NUM` | `int` | `0` | retiring | — | Former default tool count per refresh; always 0 and now dead — tools were never a live item type. | No longer has any effect; tools are not an active item type. |
| `TURRET_SPAWN_PROBABILITY` | `int` | `45` | retiring | — | Was a percentage chance of spawning any turrets in a refresh cycle; dead — turret stocking is now governed by the `item_count_ranges` shop config block. | No longer has any effect; turret availability in the shop is controlled by the shop stock configuration. |

### Shop Secondary Weapons — Quantity Scaling

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `SHOP_HEAVY_SECONDARY_SUBTYPES` | `frozenset[str]` | `{"nuke", "shock-blast", "cluster-missile"}` | global-structural | — | Classifies secondary weapon subtypes as "heavy ordnance" for the quantity scaler branch in `shop_service._generate_secondary_weapons_for_shop`; any subtype missing from both sets gets the standard scaler. | Defines which secondary weapons (nukes, shock-blasts, cluster missiles) count as heavy — they stock in smaller bundles than standard ammo. |
| `SHOP_SECONDARY_QTY_SCALER_HEAVY` | `int` | `5` | convert planned | `1–50 (proposed)` | Multiplies the rolled per-item quantity for heavy secondary subtypes in `shop_service._generate_secondary_weapons_for_shop`; keeps high-damage ordnance scarcer than standard rounds across a full refresh cycle. | Controls how many rounds of heavy weapons (nukes, shock-blasts) appear per shop refresh — lower means fewer rounds stocked. |
| `SHOP_SECONDARY_QTY_SCALER_STANDARD` | `int` | `10` | convert planned | `1–100 (proposed)` | Multiplies the rolled per-item quantity for all secondary subtypes not in `SHOP_HEAVY_SECONDARY_SUBTYPES` in `shop_service._generate_secondary_weapons_for_shop`. | Controls how many rounds of standard ammo (missiles, rockets) appear per shop refresh — higher means more rounds stocked. |

### Shop Module Buckets

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `SHOP_JUNK_MODULE_TYPES` | `frozenset[str]` | `{"TransfusionBeamModule", "ShieldInjectorModule", "TimeExtenderModule", "JumpDriveModule"}` | global-structural | — | Modules excluded entirely from shop draws in `shop_service._generate_modules_for_shop`; disjointness from the other two sets is asserted at import time as a drift guard across all 21 module types. | These module types never appear in the shop; they are reserved for future mechanics or are non-functional in the current system. |
| `SHOP_FILLER_MODULE_TYPES` | `frozenset[str]` | `{"GammaShieldModule", "SpectralFilterModule", "RepairBeamModule", "SignatureModule", "MiningDrillModule", "CompressorModule", "CabinModule"}` | global-structural | — | Non-combat modules drawn when the filler bucket is selected in `shop_service._generate_modules_for_shop`; the bucket is chosen with probability `1 − SHOP_COMBAT_MODULE_PROB`. | Utility and support modules (mining drills, cabins, signature reducers, etc.) that stock as filler when the shop does not draw a combat-tier module. |
| `SHOP_COMBAT_MODULE_TYPES` | `frozenset[str]` | `{"ScannerModule", "ArmourModule", "ShieldModule", "CloakModule", "BoosterModule", "EmergencySystemModule", "RepairBotModule", "PrimaryWeaponModModule", "ThrusterModule", "TractorBeamModule"}` | global-structural | — | Combat-priority modules drawn with probability `SHOP_COMBAT_MODULE_PROB` in `shop_service._generate_modules_for_shop`; TractorBeamModule is intentionally elevated here (it gates PvC loot) despite being a filler-class module in criminal loadouts. | Combat and loot-enabling modules (armour, cloaks, boosters, tractor beams, etc.) that are prioritised in the shop draw so players can find them reliably. |
| `SHOP_COMBAT_MODULE_PROB` | `float` | `0.75` | per-guild (wired; slash exposure planned) | `0–1` | Probability that each module slot in a shop refresh draws from `SHOP_COMBAT_MODULE_TYPES` rather than `SHOP_FILLER_MODULE_TYPES`; resolved per guild via `resolve_constant(config, "shop_combat_module_prob", …)` in `shop_service._generate_modules_for_shop`. | How often the shop stocks a combat-ready module (0 = always filler, 1 = always combat); default 0.75 means roughly 3 in 4 module slots are combat-tier. |

### Shop Tech-Level Banding

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `SHOP_BANDED_TL_WEIGHT` | `float` | `0.70` | per-guild read already live — column planned | `0–1 (proposed)` | Fraction of shop refreshes that draw their batch TL from the tier's in-band uniform range `[LO, HI]`; the remaining fraction draws from the exponential out-of-band taper; resolved via `resolve_constant` in `shop_service._pick_shop_tl`. | How reliably the shop matches your division's gear tier (0 = always off-tier, 1 = always tier-matched); default 0.70 means 7 in 10 refreshes stock items squarely in your tier range. |
| `SHOP_TL_BAND_LO_BRONZE` | `int` | `1` | per-guild read already live — column planned | `1–10 (proposed)` | Lower bound of the in-band TL range for Bronze-tier shop refreshes; consumed via `getattr(GameConstants, f"SHOP_TL_BAND_LO_{key}")` in `shop_service._pick_shop_tl`. | The lowest tech level that counts as "in tier" for Bronze shop draws; items below this appear only via the out-of-band taper. |
| `SHOP_TL_BAND_HI_BRONZE` | `int` | `2` | per-guild read already live — column planned | `1–10 (proposed)` | Upper bound of the in-band TL range for Bronze-tier shop refreshes; used alongside `SHOP_TL_BAND_LO_BRONZE` in `shop_service._pick_shop_tl`. | The highest tech level that counts as "in tier" for Bronze shop draws; items above this appear only via the out-of-band taper. |
| `SHOP_TL_BAND_LO_SILVER` | `int` | `1` | per-guild read already live — column planned | `1–10 (proposed)` | Lower bound of the in-band TL range for Silver-tier shop refreshes; same draw logic as Bronze in `shop_service._pick_shop_tl`. | The lowest tech level that counts as "in tier" for Silver shop draws. |
...
| `SHOP_UPTIER_TL_DECAY` | `float` | `0.60` | per-guild read already live — column planned | `0–1 (proposed)` | Exponential decay factor applied per TL step above the band's HI in the out-of-band taper; smaller values make higher-TL items rarer; resolved via `resolve_constant` in `shop_service._pick_shop_tl`. | How quickly above-tier items become rare in the shop (lower = steeper drop-off); default 0.60 allows moderate up-tier bleeding so players occasionally see next-tier gear. |
| `SHOP_DOWNTIER_TL_DECAY` | `float` | `0.45` | per-guild read already live — column planned | `0–1 (proposed)` | Exponential decay factor applied per TL step below the band's LO in the out-of-band taper; steeper than uptier to suppress off-tier junk; resolved via `resolve_constant` in `shop_service._pick_shop_tl`. | How quickly below-tier items become rare in the shop (lower = steeper drop-off); default 0.45 is intentionally harsher than the uptier rate to keep junk off the shelves. |

### Item Type Vocabulary

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `NUM_SHIP_RANKS` | `int` | `10` | retiring | — | Former rank-count constant used by legacy shop draw logic; dead — rank-based draws were replaced by TL-band draws and this value has no active consumer. | No longer has any effect. |
| `NUM_WEAPON_RANKS` | `int` | `10` | retiring | — | Former weapon rank count; same retirement path as `NUM_SHIP_RANKS`. | No longer has any effect. |
| `NUM_MODULE_RANKS` | `int` | `7` | retiring | — | Former module rank count; same retirement path as `NUM_SHIP_RANKS`. | No longer has any effect. |
| `NUM_TURRET_RANKS` | `int` | `3` | retiring | — | Former turret rank count; same retirement path as `NUM_SHIP_RANKS`. | No longer has any effect. |
...
| `PLAYABLE_ITEM_TYPES` | `frozenset[str]` | `{"ship", "primary_weapon", "secondary_weapon", "turret_weapon", "module", "commodity"}` | retiring | — | Formerly a distinct set intended to diverge from `CATALOG_ITEM_TYPES` once deferred mechanics shipped; currently identical, never read separately, and will be removed once issue #70 wiring is complete. | No longer has any effect; use `CURRENTLY_ENABLED_TYPES` instead. |
| `CURRENTLY_ENABLED_TYPES` | `frozenset[str]` | `{"ship", "primary_weapon", "secondary_weapon", "turret_weapon", "module", "commodity"}` | global-structural | — | Single authoritative lever gating item-type exposure across all economy and loadout flows; removing a type here disables it globally without requiring scattered if-branches; consumed in `shop_service._generate_items_for_shop` to filter `_CONCRETE_TO_CONFIG_KEY`. | The set of item types that are live in the economy today; anything not in this list is hidden from players even if data exists for it. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `LOOT_CHANCE_TRACTOR_T1` | `int` | `20` | per-guild (wired) | `0–100` | Integer percent chance of a successful loot roll when the player has a T1 tractor beam (AB-1 "Retractor") equipped; keyed into `_TRACTOR_TL_TO_CHANCE_KEY` in `loot_engine` and resolved per guild in `loot_service.tractor_chance`. | Chance of looting a kill when using the weakest tractor beam; default 20% means roughly 1 in 5 bounties yield loot. |
| `LOOT_CHANCE_TRACTOR_T2` | `int` | `40` | per-guild (wired) | `0–100` | Integer percent loot-roll chance for T2 tractor beam (AB-2 "Glue Gun"); same resolution path as T1 in `loot_engine` / `loot_service`. | Chance of looting a kill with the T2 tractor beam; default 40%. |
| `LOOT_CHANCE_TRACTOR_T3` | `int` | `60` | per-guild (wired) | `0–100` | Integer percent loot-roll chance for T3 tractor beam (AB-3 "Kingfisher"); same resolution path. | Chance of looting a kill with the T3 tractor beam; default 60%. |
| `LOOT_CHANCE_TRACTOR_T4` | `int` | `80` | per-guild (wired) | `0–100` | Integer percent loot-roll chance for T4 tractor beam (AB-4 "Octopus"); same resolution path. | Chance of looting a kill with the best tractor beam; default 80%. |
| `LOOT_CHANCE_NO_TRACTOR` | `int` | `0` | per-guild (wired) | `0–100` | Integer percent loot-roll chance when no tractor beam is equipped; resolved via `resolve_constant(guild_config, "loot_chance_no_tractor", …)` in both `loot_engine` and `loot_service`. | Chance of looting a kill with no tractor beam equipped; default 0% means loot is impossible without the module. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `DEFAULT_ACCURACY` | `float` | `1.0` | retiring (dead placeholder) | — | Legacy placeholder for shot-hit probability (1.0 = always hit); the Phase-1 `TickResolver` derives attacker accuracy from the layered §5 formula instead and never reads this constant. | This setting has no effect; it will be removed when the combat engine upgrade completes. |
| `DEFAULT_EVASION` | `float` | `0.0` | retiring (dead placeholder) | — | Legacy placeholder for dodge probability (0.0 = no evasion); the Phase-1 engine uses a hit-chance model rather than a separate evasion track, so this constant is unreferenced. | This setting has no effect; it will be removed when the combat engine upgrade completes. |
| `CLOAK_ACCURACY_PENALTY` | `float` | `0.0` | retiring (dead placeholder) | — | Additive accuracy-penalty placeholder applied when the target is cloaked (0.0 = no effect); superseded by `CLOAK_SET_VALUE`, which replaces the whole §5 accuracy formula while the cloak is active. | This setting has no effect; cloak strength is now handled by a separate mechanism. |
| `SCANNER_ACCURACY_BONUS` | `float` | `0.0` | retiring (dead placeholder) | — | Flat accuracy-bonus placeholder for any equipped scanner (0.0 = no bonus); superseded by the tier-based `SCANNER_TIER_B_BONUS_PP` / `SCANNER_TIER_C_BONUS_PP` constants in the Phase-1 engine. | This setting has no effect; scanner bonuses are now applied per scanner tier. |
| `THRUSTER_EVASION_BONUS` | `float` | `0.0` | retiring (dead placeholder) | — | Evasion-bonus placeholder for equipped thruster modules (0.0 = no bonus); superseded by `THRUSTER_ACCURACY_BONUS_FACTOR`, which boosts the attacker's own accuracy at close range rather than granting evasion. | This setting has no effect; thrusters now improve a ship's own aiming rather than its ability to dodge. |
| `SHIELD_RECHARGE_RATE` | `float` | `0.0` | retiring (dead placeholder) | — | Global shield-regeneration rate placeholder (0.0 = disabled); the Phase-1 engine derives each shield module's recharge schedule from its per-item `shield_recharge_ms` seed value, not from this constant. | This setting has no effect; shield recharge speed is determined by each shield module's own stats. |
| `REPAIR_BOT_HEAL_RATE` | `float` | `0.0` | retiring (dead placeholder) | — | Repair-bot heal-rate placeholder (0.0 = disabled); superseded by `KETAR_I_REPAIR_PCT_PER_SEC` / `KETAR_II_REPAIR_PCT_PER_SEC`, which are detected per module type in `TickResolver._init_combatant()`. | This setting has no effect; repair-bot heal rates are now set per robot tier. |
| `BOOSTER_DPS_MULTIPLIER` | `float` | `1.0` | retiring (dead placeholder) | — | DPS-multiplier placeholder while a booster is active (1.0 = neutral); superseded by the Phase-1 booster mechanics — distance push via `effect_pct` and opponent accuracy debuff via `BOOSTER_ACCURACY_DEBUFF_FACTOR`. | This setting has no effect; boosters now push ships apart and reduce enemy aim rather than multiplying damage. |
| `COMBAT_TICK_RATE` | `float` | `1.0` | retiring (dead placeholder) | — | Tick-frequency-multiplier placeholder (1.0 = no change); the Phase-1 resolver uses a fixed 10 ms tick from `TICK_MS` with no per-guild rate scaling. | This setting has no effect; the combat simulation runs at a fixed internal speed. |
| `PERSISTENT_DAMAGE_DECAY_RATE` | `float` | `0.0` | retiring (dead placeholder) | — | Cross-fight damage-persistence decay placeholder (0.0 = instant full heal between fights); the Phase-1 engine always starts combatants at full HP — damaged-start support is Phase-2. | This setting has no effect; ships always enter combat at full health in the current game. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `NUKE_RANGE_REGIME_THRESHOLD_M` | `int` | `1000` | convert planned | `ge=0, le=10000` (proposed) | Distance boundary (metres) separating long-range from close-range nuke detonation windows in `_nuke_detonation_window()` (`combat_resolver.py`); above threshold the `NUKE_LR_NEAR_FRAC` window applies; at or below, the CR bracket applies. | Sets where nukes switch from a targeted far-shot pattern to a close-range artillery bracket — lower means the bracket applies across more of the fight. |
| `NUKE_LR_NEAR_FRAC` | `float` | `0.40` | convert planned | `ge=0.0, le=1.0` (proposed) | Long-range epicentre window near-edge as a fraction of current distance (`window = [NEAR_FRAC × d, d]`); controls how close the nearest possible epicentre can be to the firer when shooting long-range; used in `_nuke_detonation_window()`. | Controls long-range nuke self-risk — lower moves the near edge toward the firer, making nukes riskier to both sides at long distance. |
| `NUKE_CR_SHORT_M` | `int` | `600` | convert planned | `ge=0, le=5000` (proposed) | Close-range epicentre window short-edge offset (metres); window opens at `max(0, d − NUKE_CR_SHORT_M)`, letting the blast land short of the target; used in `_nuke_detonation_window()`. | Controls how far short of the target a close-range nuke can land — higher means wider spread toward the firer's own position. |
| `NUKE_CR_OVERSHOOT_M` | `int` | `400` | convert planned | `ge=0, le=5000` (proposed) | Close-range epicentre window far-edge offset (metres); window extends to `d + NUKE_CR_OVERSHOOT_M`, allowing overshoot past the target; used in `_nuke_detonation_window()`. | Controls how far past the target a close-range nuke can overshoot — higher means wider spread on the far side. |
| `NUKE_STACK_FALLOFF` | `float` | `0.5` | convert planned | `ge=0.0, le=1.0` (proposed) | Per-detonation yield interference multiplier (`stack_mult = falloff ** prior_detonations_this_side`); each successive nuke fired by one side multiplies total yield so stacking many nukes gives diminishing returns; applied in `TickResolver.resolve()`. | Controls how fast repeated nuke use loses impact — lower means loading many nukes gives far less extra damage than a single well-placed shot. |
| `SHOCK_BLAST_TRIGGER_RANGE_M` | `int` | `500` | convert planned | `ge=0, le=10000` (proposed) | Maximum range (metres) at which a shock-blast secondary will fire; beyond this range the weapon holds its cooldown to avoid wasting a round on a pointless distance reset; checked per tick in `TickResolver.resolve()`. | Sets how close ships must be before a shock-blast fires — lower means it only activates very close up; higher lets it push ships apart from further away. |
