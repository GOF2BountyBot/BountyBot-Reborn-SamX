...
def test_clear_cache_removes_dirs(tmp_path: Path) -> None:
...
def test_clear_cache_returns_freed_mb(tmp_path: Path) -> None:
    ...
    with patch("glob.glob", return_value=[str(dir1)]), TestClient(app) as client:
        response = client.post("/api/v1/cache/clear")
...
def test_clear_cache_counts_errors_on_rmtree_failure(tmp_path: Path) -> None:
    ...
    with (
        patch("glob.glob", return_value=[str(dir1)]),
...
def test_cache_stats_counts_dirs(tmp_path: Path) -> None:
    ...
    with patch("glob.glob", return_value=[str(dir1), str(dir2)]), TestClient(app) as client:
        response = client.get("/api/v1/cache/stats")
...
def test_cache_stats_returns_total_mb_rounded(tmp_path: Path) -> None:
...
def test_cache_stats_handles_stat_error(tmp_path: Path) -> None:
    """GET /cache/stats should not crash if iterating a directory raises an exception."""
    # Create a real dir so glob returns it, but patch Path.rglob to raise
    dir1 = tmp_path / "blender_render_gone"
    dir1.mkdir()

    with (
        patch("glob.glob", return_value=[str(dir1)]),
        patch("pathlib.Path.rglob", side_effect=OSError("Permission denied")),
        TestClient(app) as client,
    ):
        response = client.get("/api/v1/cache/stats")
