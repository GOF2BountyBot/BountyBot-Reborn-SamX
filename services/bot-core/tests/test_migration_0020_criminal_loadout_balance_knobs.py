"""Migration tests for 0020_criminal_loadout_balance_knobs (BALANCE_JOURNAL §A).

Verifies that:
- upgrade() adds all 7 nullable per-guild override columns to guild_configs
  (idempotent via inspector guard)
- downgrade() drops all 7 columns (idempotent via inspector guard)
- The REAL upgrade()/downgrade() functions run without error (smoke test)
- Re-running upgrade()/downgrade() is a safe no-op

SQLite approach (matching test_migration_0019 pattern):
- Structural tests use an in-memory SQLite engine with a minimal guild_configs
  table lacking the new columns.
...
- The new columns are nullable and untyped-for-our-purposes; what matters is
  that the inspector sees each column present or absent.

Postgres path (matching test_migration_0019 pattern):
- When the Postgres test DB is reachable, additional tests run directly against
  ...
  verify the columns are nullable. These leave ZERO trace on the live DB.
"""
...
_MIGRATION_PATH = os.path.abspath(
    os.path.join(
        ...
        "revisions",
        "versions",
        "0020_criminal_loadout_balance_knobs.py",
    )
)


def _load_migration_module():
    """Load the 0020 migration module via importlib (avoids Alembic env)."""
    spec = importlib.util.spec_from_file_location("migration_0020_real", _MIGRATION_PATH)
...
def _create_guild_configs_table(engine: sa.engine.Engine) -> None:
    """Create a minimal guild_configs table WITHOUT the new override columns."""
    meta = sa.MetaData()
    sa.Table(
        _TABLE,
        meta,
...
class TestMigration0020Structure:
    """0020 structural tests using in-memory SQLite.

    These verify add/drop idempotency and column existence without requiring
    a live Postgres instance.
    """

    def test_upgrade_adds_all_seven_columns(self):
        """upgrade() adds all 7 criminal-loadout override columns."""
    ...
    def test_downgrade_drops_all_seven_columns(self):
        """downgrade() removes all 7 override columns."""
        engine = create_engine("sqlite:///:memory:")
        _create_guild_configs_table(engine)
...
def _build_mock_op_pg(conn: sa.engine.Connection) -> MagicMock:
    ...
    Renders the real column type so JSON/JSONB columns add correctly. All
    columns are nullable in 0020, so no NOT NULL/DEFAULT handling is needed.
    """
    mock_op = MagicMock()
    mock_op.get_bind.return_value = conn
...
@_PG_MARK
class TestMigration0020Postgres:
    """0020 Postgres tests — run against the live Postgres, always rolled back.
    ...
    def test_upgrade_columns_are_nullable(self, pg_engine):
        """After upgrade(), all 7 columns exist and are nullable in information_schema."""
        mod = _load_migration_module()
    ...
    def test_downgrade_drops_all_columns(self, pg_engine):
        """After upgrade() + downgrade(), all 7 columns are absent."""
        mod = _load_migration_module()
