...
async def get_role(request: Request, role_id: int) -> RoleResponse:
    ...
    try:
        ...
        return RoleResponse(status="success", data=role_data)
...
async def update_role(request: Request, role_id: int, role_data: RoleUpdateRequest) -> RoleResponse:
    ...
    try:
        ...
        return RoleResponse(status="updated", data=updated_role_data)
    except HTTPException:
        raise
    except Exception as exc:  # pylint: disable=broad-exception-caught
...
async def assign_role_to_user(request: Request, role_id: int, user_id: int) -> SuccessResponse:
    ...
    try:
        ...
        if not member:
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail=f"Member {user_id} not found in guild {guild.id}"
                ) from exc
        ...
        flogger.info(message)
        return SuccessResponse(status="assigned", message=message)
    except HTTPException:
        raise
    except Exception as exc:  # pylint: disable=broad-exception-caught
...
async def remove_role_from_user(request: Request, role_id: int, user_id: int) -> SuccessResponse:
    ...
    try:
        ...
        flogger.info(message)
        return SuccessResponse(status="removed", message=message)
    except HTTPException:
        raise
    except Exception as exc:  # pylint: disable=broad-exception-caught
...
async def check_user_has_role(request: Request, role_id: int, user_id: int) -> PermissionCheckResponse:
    ...
    try:
        ...
        return PermissionCheckResponse(status="success", data={"allowed": has_role})
    except HTTPException:
        raise
    except Exception as exc:  # pylint: disable=broad-exception-caught
