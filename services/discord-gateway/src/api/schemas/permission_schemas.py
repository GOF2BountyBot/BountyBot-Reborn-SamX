...
class PermissionOverwrite(BaseModel):
    ...
    id: str | None = Field(None, description="Permission overwrite ID (channel_id:target_id)")
    # Make channel_id optional so channel-scoped endpoints can omit it
    channel_id: int | None = Field(None, description="Channel ID (optional when used inside channel-scoped requests)")
...
class PermissionOverwriteRequest(BaseUpdateRequest):
    ...
    allow: int | None = Field(None, ge=0, description="Permissions to allow (bitfield)")
    deny: int | None = Field(None, ge=0, description="Permissions to deny (bitfield)")
...
class CalculatePermissionsRequest(BaseModel):
    ...
    allow: int | None = Field(None, ge=0, description="Allow overwrite bitfield")
    deny: int | None = Field(None, ge=0, description="Deny overwrite bitfield")
...
class PermissionScope(BaseModel):
    ...
    id: int | None = Field(
        None,
        description=(
            "ID of the scope (guild_id, channel_id, category_id, or thread_id). Required for non-guild scopes"
        ),
    )
...
class CanDoResponseData(BaseModel):
    ...
    breakdown: list[PermissionCheckResult] | None = Field(
...
class PermissionGrantSource(BaseModel):
    ...
    role_name: str | None = Field(None, description="Name of the role that granted the permission (if type is 'role')")
    role_id: int | None = Field(None, description="ID of the role that granted the permission (if type is 'role')")
...
class PermissionTarget(BaseModel):
    ...
    id: int | None = Field(
        None,
        description=(
            "Entity ID of the target (guild_id, channel_id, category_id, or thread_id). Required for non-guild targets."
        ),
    )
