...
- **Loot inventory-cache invalidation:** because loot is written to the player's inventory **server-side** during `/check` (bot-core `_apply_loot_on_win`), there is no gateway mutation command to hook — so the cog explicitly calls `autocomplete_state.invalidate_inventory(guild_id, player_id)` when any outcome's `loot.outcome ∈ {looted, partial}`. Without this the `/sell` autocomplete (a `peek`, which never re-fetches on its own) showed stale data until the 5-min round-robin warmer or the next inventory-mutating command. `failed`/`cargo_full`/`none` add nothing, so they do not invalidate.
