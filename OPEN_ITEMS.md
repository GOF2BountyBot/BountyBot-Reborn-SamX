...
| TRUEUP-P5 | 🟠 | `DatabaseManager.table_exists()` always returns `False` — calls sync SQLAlchemy `inspect()` from `async def` with no `run_sync` bridge; the resulting `MissingGreenlet` is swallowed by its own `except SQLAlchemyError` | Found 2026-07-22 during TRUEUP-05; verified against real aiosqlite AND real asyncpg engines. Call-site impact audit pending. Full writeup in `TEST_SUITE_TRUEUP_FOLLOWUPS.md` (R-bc-db-manager); a demonstration test documents observed behavior |
