...
class PlayerShipRepository(IRepository[PlayerShip]):
    ...
    async def get_by_id(self, db: AsyncSession, ship_id: int) -> Optional[PlayerShip]:
        """Get ship by ID."""
        try:
            return await db.get(PlayerShip, ship_id)
        except Exception as e:
            flogger.error(f"Error getting ship by ID {ship_id}: {e}")
            raise
    ...
    async def add(self, db: AsyncSession, ship: PlayerShip) -> PlayerShip:
        """Add new ship to database."""
        try:
            db.add(ship)
            await db.commit()
            await db.refresh(ship)
            flogger.info(f"Added ship: {ship.ship_name} for player {ship.player_id}")
            return ship
        except Exception as e:
    ...
    async def remove(self, db: AsyncSession, ship: PlayerShip) -> None:
        """Remove ship from database."""
        try:
            await db.delete(ship)
            await db.commit()
            flogger.info(f"Removed ship: {ship.id}")
        except Exception as e:
            flogger.error(f"Error removing ship {ship.id}: {e}")
            await db.rollback()
            raise
