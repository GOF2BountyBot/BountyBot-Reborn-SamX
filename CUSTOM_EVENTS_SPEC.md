...
| Command | Notes |
|---|---|
| `/admin_event_create type duration_days [params] [save_as_template template_name] [template]` | draft. **Templates (user, 2026-09-05):** a sixth event state `template` with a per-guild unique `name` (migration 0037, partial unique index). `save_as_template:True` + `template_name` stores the row as a template (bool picker + name, user 2026-09-05); `type:"From Template"` (pseudo slug `from_template`) + `template:<id>` creates a draft copying the template's type/params/duration and every prize row — explicit create options win over the template. Templates are edited with the same edit/prize/view/delete commands (state gates accept `template`), never start (start/tick/player selectors exclude them), and show under `/admin_event_list state:template`. Prize/edit/delete state gates: draft/template (+active for add_prize). **OOB templates (user, 2026-09-05):** `services/event_templates.json` → `sync_guild_templates()` on first config creation (`ConfigService.create_or_update_config`, i.e. `/admin_setup`) and `sync_all_guild_templates()` at startup (fallback/upgrade). Match by (guild, state=template, name); insert when missing; overwrite type/params/duration/prizes only while `created_by_user_id IS NULL` and `created_at == updated_at` (add/delete prize and edit all bump `updated_at`); otherwise skip. Deleted → re-seeded next startup. |
...
| `/admin_event_add_prize event place type [item] qty` | `place` choices: `1st…10th`, `Top N` (+`top_n`), `Participation`; `item` autocomplete filtered by `type` via `interaction.namespace`; validates catalog + overlap on add. Drafts + active (+templates). **Occupied place → confirm dialog, then `replace: true`** (user, 2026-09-05: swap prizes on a draft cloned from a template without remove-then-add; the ConfirmView pattern). |
