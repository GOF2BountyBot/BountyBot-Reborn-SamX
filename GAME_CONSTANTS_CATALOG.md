...
| `DIVISION_MAX_TL` | dict[str, int] | `{bronze:2, silver:4, gold:7, platinum:10}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to division_max_tl_{bronze,silver,gold,platinum} in 0030. Derived dict rebuilt from scalars in GameConstants.load(). | Use the per-division scalar settings (division_max_tl_bronze, etc.) instead. |
...
| `BOUNTY_DIVISION_REWARD_MULT` | dict[str, float] | `{bronze:1.0, silver:2.0, gold:1.0, platinum:1.0}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to bounty_division_reward_mult_{bronze,...} in 0030. Derived dict rebuilt from scalars in GameConstants.load(). | Use the per-division scalar settings (bounty_division_reward_mult_bronze, etc.) instead. |
...
| `PRIMARY_TL_BAND_WEIGHTS` | dict[str, int] | `{center:70, minus1:20, plus1:10}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to primary_tl_band_weight_{center,minus1,plus1} in 0030. Derived dict rebuilt from scalars in GameConstants.load(). | Use the per-key scalar settings (primary_tl_band_weight_center, etc.) instead. |

### Criminal Loadout — Module Gate Chances

| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `CRIMINAL_CLOAK_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:0, silver:25, gold:66, platinum:100}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to criminal_cloak_chance_{bronze,...} in 0030. Derived dict rebuilt from scalars in GameConstants.load(). | Use the per-division scalar settings (criminal_cloak_chance_bronze, etc.) instead. |
| `CRIMINAL_BOOSTER_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:50, silver:100, gold:100, platinum:100}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to criminal_booster_chance_{bronze,...} in 0030. | Use the per-division scalar settings (criminal_booster_chance_bronze, etc.) instead. |
| `CRIMINAL_EMERGENCY_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:0, silver:25, gold:50, platinum:100}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to criminal_emergency_chance_{bronze,...} in 0030. | Use the per-division scalar settings (criminal_emergency_chance_bronze, etc.) instead. |
| `CRIMINAL_WEAPONMOD_CHANCE_BY_DIVISION` | dict[str, int] (%) | `{bronze:0, silver:25, gold:50, platinum:100}` | **Retired (rev 0033)** — flattened to per-key scalars | — | JSONB column dropped in migration 0033; backfilled to criminal_weaponmod_chance_{bronze,...} in 0030. | Use the per-division scalar settings (criminal_weaponmod_chance_bronze, etc.) instead. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `KETAR_I_REPAIR_PCT_PER_SEC` | `float` | `0.02` | global (A2 deferred by owner; column retained, guard-allowlisted) | `ge=0.0, le=1.0` (proposed) | Per-second hull+armour heal rate (fraction of combined max HP) for the Ketar Repair Bot I module; consumed in `loadout_builder.py` when building a ship's runtime module list: if the module is a `_REPAIR_BOT_MODULE_TYPE` and has no explicit `repair_pct_per_sec` seed key, falls back to this constant as the default Ketar I rate; the resulting `repair_rate` attribute is then read by `_init_combatant()` in `combat_resolver.py` and applied through an integer-flush accumulator each tick. 2 %/s is the owner-confirmed tuned value (spec corrected 2026-08-26 from its stale 2.5). | Controls how fast the basic repair bot heals hull and armour during combat — higher means faster in-fight recovery. |
| `KETAR_II_REPAIR_PCT_PER_SEC` | `float` | `0.04` | global (A2 deferred by owner; column retained, guard-allowlisted) | `ge=0.0, le=1.0` (proposed) | Per-second hull+armour heal rate (fraction of combined max HP) for the Ketar Repair Bot II module; consumed in `loadout_builder.py` when building a ship's runtime module list: if the module is a `_REPAIR_BOT_MODULE_TYPE` and has no explicit `repair_pct_per_sec` seed key, falls back to this constant as the default Ketar II rate; the resulting `repair_rate` attribute is then read by `_init_combatant()` in `combat_resolver.py` and applied through an integer-flush accumulator each tick. 4 %/s is the owner-confirmed tuned value (spec corrected 2026-08-26 from its stale 5.0). | Controls how fast the advanced repair bot heals hull and armour — higher means better combat survival for ships that equip it. |
