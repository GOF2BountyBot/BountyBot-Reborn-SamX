...
def has_channel_permission(
    ...
    try:
        ...
        flogger.warning(
            f"Error checking channel permission for member {member_id} in channel {channel_id}: {exc}"
        )
        return False
...
def evaluate_user_guild_permissions(
    ...
    """
    flogger.debug(
        f"Evaluating guild permissions for member {getattr(member, 'id', 'unknown')}; "
...
def evaluate_role_guild_permissions(
    ...
    flogger.debug(
        f"Evaluating guild permissions for role {role_name} (id={role_id}); "
        f"requested={requested_permissions}"
    )
...
def evaluate_role_channel_permissions(
    ...
    try:
        ...
        flogger.error(
            f"Error evaluating channel permissions for role {role_name} (id={role_id}) in "
...
def _find_permission_source(member: "Any", permission: str) -> PermissionSource:
    ...
    for role in reversed(member.roles):
        ...
        if role_perms & perm_bit:
            ...
            role_id = getattr(role, "id", "unknown")
            flogger.debug(
                f"Permission source for member {member_id} ({permission}): role {role_name} (id={role_id})"
            )
            return PermissionSource("role", role.name, role.id)
...
def _find_channel_permission_source(member: "Any", channel: "Any", permission: str) -> PermissionSource:
    ...
    channel_id = getattr(channel, "id", "unknown")
    flogger.debug(
        f"Finding channel permission source for member {member_id} in channel {channel_id} ({permission})"
    )
            ...
            if target_id is not None and target_id == member_id_val:
                try:
                    ...
                    if getattr(allow_perm, "value", 0) & perm_bit:
                        flogger.debug(
                            f"Found direct member overwrite for {member_id} in {channel_id} ({permission})"
                        )
            ...
            if role_overwrite:
                try:
                    ...
                    if getattr(allow_perm, "value", 0) & perm_bit:
                        if role_name == "@everyone":
                            flogger.debug(
                                f"Found role overwrite for @everyone in {channel_id} ({permission})"
                            )
                            return PermissionSource("everyone")
                        flogger.debug(
