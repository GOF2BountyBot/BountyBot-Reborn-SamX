...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `MAX_BOUNTIES_PER_DIVISION` | int | 5 | retiring (dead) | — | Never read by live code — the spawn executor caps per tier via the per-guild `bounty_max_per_tier` JSON (fallback 3); `TemperatureService.get_max_bounties()` has zero callers. | no effect; per-tier bounty caps are set via the bounty config. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `GUILD_ACTIVITY_DECAY_RATE` | float | ~0.667 (2/3) | per-guild (wired) — NOTE: vestigial subsystem | 0.0–1.0 | Geometric decay multiplier applied once per GUILD_ACTIVITY_DECAY_INTERVAL in `temperature_service.decay_temperature`: `temperature ← max(min_activity, round(temperature × rate, 1))`; resolved via resolve_constant. The decay job is the only live consumer; nothing raises temperature (`raise_temperature` has zero callers) and nothing reads temperature for spawning, so this knob currently has no player-visible effect pending an owner decision to revive or remove the temperature subsystem. | How fast guild activity cools down when no /check uses happen — lower values mean bounty spawning slows more aggressively during quiet periods. NOTE: vestigial subsystem — currently has no player-visible effect. |
| `MIN_GUILD_ACTIVITY` | float | 1.0 | per-guild (wired) — NOTE: vestigial subsystem | 0.0–100.0 | Floor clamp applied after each decay step in `temperature_service.decay_temperature`. Also read by the dead `temperature_service.calculate_spawn_delay()` as a guard, but that function is not called in production. The decay job is the only live consumer; nothing raises temperature (`raise_temperature` has zero callers) and nothing reads temperature for spawning, so this knob currently has no player-visible effect pending an owner decision to revive or remove the temperature subsystem. | The minimum activity level your guild maintains even during long quiet periods — prevents bounty spawning from stopping entirely. NOTE: vestigial subsystem — currently has no player-visible effect. |
| `ACTIVITY_TEMP_PER_PLAYER` | int | 1 | owner-flag (vestigial subsystem) | 0–100 | Activity increment per /check use; `temperature_service.raise_temperature` reads `GameConstants.ACTIVITY_TEMP_PER_PLAYER` directly (not via resolve_constant), and `raise_temperature` itself has no live production callers — /check no longer raises temperature. Both the wiring fix and the call-site restoration are needed before this has any effect. | currently has no effect. |
...
| Constant | Type | Default | Scope | Bounds | Technical | Admin-facing |
|---|---|---|---|---|---|---|
| `GUILD_ACTIVITY_DECAY_INTERVAL` | int (seconds) | 3600 | retiring (dead) | — | Default 3600 s (1 hour). Not wired to any APScheduler trigger; the temperature-decay job cron is hardcoded `"0 * * * *"` in main.py. The env var `BOUNTYBOT_GUILD_ACTIVITY_DECAY_INTERVAL` updates `GameConstants.GUILD_ACTIVITY_DECAY_INTERVAL` but nothing reads it for scheduling. Effectively documentary-only; the actual decay cadence is controlled by the hardcoded cron. | no effect. |
...
| `NUKE_CR_OVERSHOOT_M` | `int` | `400` | convert planned | `ge=0, le=5000` (proposed) | Close-range epicentre window far-edge offset (metres); window extends to `d + NUKE_CR_OVERSHOOT_M`, allowing overshoot past the target; used in `_nuke_window()`. | Controls how far past the target a close-range nuke can overshoot — higher means wider spread on the far side. |
